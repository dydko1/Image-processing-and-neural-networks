...
if (sum(wskazanykataloG)~=0)
    [nazwapliku,docelowykatalog] = uiputfile('*.mat','Plik z Wynikami i Zrzuty wykresów','wyniki');
...
%%
        for n = 1:Nplikow            
            nazwapliku=(zlistowane(n).name);
            koncowka=nazwapliku(end-5:end-4);
            load ([wskazanykataloG,'\',nazwapliku]);
%%
...
%%          
            ...
            % nazwa=[docelowykatalog,'\',podkatalog,'\siec_razem_',koncowka,'.jpg'];
...
%%            
%%            
            wynik=[wynik; pop,rozp,npop,nrozp,sprawnosc];
            nazwysieci=[nazwysieci;nazwasieci]; % uwaga sieci 
        end
%%
        plikdozapisu=strcat(podkatalog,'\',nazwapliku);
        naglowek=char(['pop         rozp         npop         nrozp         sprawnosc']);
        wyniki=char([num2str(wynik),nazwysieci]);
        [i1,j1]=size(naglowek);        
        [i1,j2]=size(wyniki);        
        if (j1<j2)
            ile=j2-j1;
...
%%
            naglowek=[naglowek pom];
            dopliku=strcat([naglowek ; wyniki]);
%%
            save(plikdozapisu,'dopliku');
%%
        else
%%
            save(plikdozapisu,'wyniki');
%%
        end
