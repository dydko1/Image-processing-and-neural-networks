function varargout = hurtowo2_1_1(varargin)
% HURTOWO2_1_1 M-file for hurtowo2_1_1.fig
%      HURTOWO2_1_1, by itself, creates a new HURTOWO2_1_1 or raises the existing
%      singleton*.
%
%      H = HURTOWO2_1_1 returns the handle to a new HURTOWO2_1_1 or the handle to
%      the existing singleton*.
%
%      HURTOWO2_1_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HURTOWO2_1_1.M with the given input arguments.
%
%      HURTOWO2_1_1('Property','Value',...) creates a new HURTOWO2_1_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before hurtowo2_1_1_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to hurtowo2_1_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help hurtowo2_1_1

% Last Modified by GUIDE v2.5 17-May-2007 11:12:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @hurtowo2_1_1_OpeningFcn, ...
    'gui_OutputFcn',  @hurtowo2_1_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before hurtowo2_1_1 is made visible.
function hurtowo2_1_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to hurtowo2_1_1 (see VARARGIN)

% Choose default command line output for hurtowo2_1_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes hurtowo2_1_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = hurtowo2_1_1_OutputFcn(hObject, eventdata, handles)
...
[wskazanykataloG] = uigetdir;
if (sum(wskazanykataloG)~=0)
    [docelowykataloG] = uigetdir(wskazanykataloG);
    if (sum(docelowykataloG)~=0)
        % ---------------------------------------------------- %
        pyt{1} = 'Szerokosc obrazu (Szer i Wyso-0- bez zmian)';      % tekst przy polu do wprowadzenia zmiennej 1
        pyt{2} = 'Wysokosc obrazu (Szer i Wyso-0- bez zmian)';       % tekst przy polu do wprowadzenia zmiennej 2
...
if (sum(wskazanykataloG)~=0)
    ...
    if (sum(docelowykataloG)~=0)
        ...
        %  a inaczej ja przerobilem w hurtowo2_1_1.m
        % 
        % ---------------------------------------------------- %
...
%%
                    ...
                    for(m=1:X:j)
                        for (n=1:Y:i)
                            ...
                            plikdozapisu=strcat(docelowypodkataloG,docelowanazwa,nazwapliku);
            ...
            for n=1:dlugosc
                if (katalogi(n).isdir)&(~(strcmp(katalogi(n).name,'.')) & ~(strcmp(katalogi(n).name,'..')))
                    ...
                    for m = 1:Nplikow
                        ...
                        plot(Ytest,'r+')
                        axis([0 200 -1.3 1.3])
                        title('-1 - reszta, 1 - dobry ')
                        xlabel('Numer wzorca')
                        ylabel('Ksztalt')
                        legend('prawdziwy ksztalt','rozpoznany ksztalt')
                        set(gca,'YTick',[-1 -1+blad 0 1-blad 1]);
                        set(gca,'YGrid','on');
                        %%
...
domowy=pwd; %##% pobiera sciezke do katalogu domoweg
%pyt{1} = 'Podaj bledy (.1,.15,.2,.25,.3)';      % tekst przy polu do wprowadzenia zmiennej 1
pyt{1} = 'Podaj bledy (.1,.15,.2,.25,.3)';
...
%[docelowykatalog] = uigetdir(zrodlowykatalog); jak ma zaczynac od zrodlowykatalog
    [docelowykatalog] = uigetdir(zrodlowykatalog);
    if (sum(docelowykatalog)~=0)
